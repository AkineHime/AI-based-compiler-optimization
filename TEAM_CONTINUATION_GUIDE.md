...
Benchmarks emitted to C and compiled with **`gcc -O0`**, swept over all **64
pass combinations** (6 toggleable passes: CF, DCE, CSE, LICM, SR, LU). Every
timed config is verified to reproduce the unoptimized baseline's exit code
before its time is recorded. See `RESULTS.md` for the current numbers (the model
bake-off and per-program table are regenerated by `run_experiment.py`).
