...
Work is split into four roles (see `persons_work_dist.md`). Every commit carries
...
| Correctness harness (64-combo regression) | `tests/test_correctness*.py`, `tests/test_loop_unroll.py` | Person B |
...
| ML dataset, model bake-off, GroupKFold training, predictor | `src/minic/ml/` | Person C |
| CLI + browser playground | `demo/` | Person C |
| Results report + chart + published page | `src/minic/harness/report.py`, `src/minic/harness/plot.py`, `docs/` | Person C |
| Hand-written benchmark corpus, ground truth | `benchmarks/` | Person D |
| Report sections (MiniC design rationale) | — | Person D |

Cross-cutting fixes (Python 3.13 imports in Part A; several latent optimizer
bugs surfaced while integrating later passes) were made in place with the
relevant area's owner credited and documented in `PERSON_B_NOTES.md` /
`PERSON_C_NOTES.md`.
